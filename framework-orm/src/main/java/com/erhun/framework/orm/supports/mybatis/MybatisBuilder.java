...
import com.erhun.framework.orm.AttributeInfo;
import com.erhun.framework.orm.SQLUtils;
import com.erhun.framework.orm.annotations.*;
...
import com.erhun.framework.orm.entities.IOrderEntity;
...
public final class MybatisBuilder {
    ...
    private static class Builder{
        ...
        private void buildQueryCriteriaSQL(StringBuilder buf) {
            ...
            buf.append("select ");
            buf.append("<choose><when test=\"criteria.criteria.fetchFields!=null and criteria.criteria.fetchFields.length()&gt;0\">${criteria.fetchFields}</when>");
            buf.append("<otherwise>");
            for (AttributeInfo field : entityFields) {
                if (field.isQueryable()) {
